...
export function buildDynamicStorySection(scene, mainContainer) {
    ...
    textGroupsData.forEach(groupData => {
        ...
        // --- CORRECCIÓN DEFINITIVA DE LA CONDICIÓN DEL ICONO ---
        // Se comprueba "groupData.icon" directamente, en lugar de "groupData.icon.data"
        else if (groupData.animation_type === 'icon-hand-float' && groupData.icon) {
             const iconData = getAttrs(groupData.icon); // Se usa "groupData.icon"
