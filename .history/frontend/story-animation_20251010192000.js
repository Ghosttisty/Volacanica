...
export function buildDynamicStorySection(scene, mainContainer) {
    ...
    const styles = `
        ...
        /* --- AJUSTE DE POSICIÓN DEL ICONO --- */
        .icon-group { justify-content: center; }
        .icon-group .home-animate__icon {
            position: absolute;
            top: -100%;
            left: 50%;
    ...
    requestAnimationFrame(() => {
        ...
        // --- CORRECCIÓN DE REPETICIÓN DE TYPEWRITER ---
        gsap.utils.toArray('.typewriter-group').forEach(group => {
            ...
            ScrollTrigger.create({
                ...
                containerAnimation: scrollTl,
                start: "left 75%",
                // Callbacks para controlar la animación en todas las direcciones
                onEnter: () => {
                    group.textContent = ''; // Limpia el texto
                    tween.restart(); // Reinicia y ejecuta la animación
                },
                onEnterBack: () => {
                    group.textContent = '';
                    tween.restart();
                ...
                onLeave: () => {
                    // Asegura que el texto esté completo al pasar de largo
                    gsap.set(group, {text: {value: text}});
                },
                onLeaveBack: () => {
                    // Limpia el texto al salir de la pantalla hacia atrás
                    group.textContent = '';
                }
            });
        });
