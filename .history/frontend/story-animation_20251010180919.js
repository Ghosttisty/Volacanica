...
export function buildDynamicStorySection(scene, mainContainer) {
    ...
    // --- PASO 1: Inyectar los estilos EXACTOS del código auxiliar ---
    const styles = `
        ...
        .home-animate__text-group{ align-items:center; display:flex; font-size:max(5.625rem,min(5.58824vw + 1.29412rem,8rem)); font-weight:600; line-height:1; position:relative; margin: 0 8vw; white-space:nowrap; }
    ...
    // --- PASO 2: Construir la estructura HTML exacta ---
    ...
    textGroupsData.forEach(groupData => {
        ...
        if (groupData.animation_type === 'nice-group-entry') {
            groupHTML = `
                <p class="home-animate__text-group home-animate__text-group--nice" aria-hidden="true">
            ...
            `;
        } else { // Para 'default-fade-in' y otros
            groupHTML = `
                <p class="home-animate__text-group default-group" style="opacity: 0;">
                   ${groupData.text_1 || ''}
                </p>
             `;
        }
    ...
    // --- PASO 3: Ejecutar la lógica GSAP exacta del código auxiliar ---
    requestAnimationFrame(() => {
        ...
        // Corrección CRÍTICA: Posicionar la tira de texto fuera de la pantalla al inicio
        ...
        const scrollTl = gsap.timeline({
            scrollTrigger: {
                trigger: trigger,
                start: "top top",
                end: () => `+=${trackEl.scrollWidth * 1.5}`, // Multiplicador para dar más espacio de scroll
                scrub: 1,
                pin: true,
                anticipatePin: 1,
        ...
        scrollTl.to(trackEl, { x: () => -(trackEl.scrollWidth), ease: "none" });

        // Recreación de las animaciones del auxiliar
        const animConfigs=[
        ...
        // Animación para los grupos por defecto
        gsap.utils.toArray('.default-group').forEach(group => {
            gsap.to(group, {
                opacity: 1,
                scrollTrigger: {
                    trigger: group,
