...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let slides =[]; // <-- CORRECCIÓN VERIFICADA
    ...
    async function loadHeroSlides() {
        try {
            // Usamos?populate=* para asegurarnos de que la imagen venga incluida.
            ...
            if (slides && slides.length > 0) {
                slides.forEach((slideData) => {
                    ...
                    // CORRECCIÓN: Verificamos que la estructura de la imagen exista antes de usarla
    ...
    async function loadMenu() {
        try {
            // CORRECCIÓN: Usamos una sintaxis de populate más robusta para anidación.
            const response = await fetch(`${API_URL}/categories?populate[menu_items][populate]=*`);
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            const data = await response.json();
