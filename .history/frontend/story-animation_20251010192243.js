...
export function buildDynamicStorySection(scene, mainContainer) {
    ...
    requestAnimationFrame(() => {
        ...
        // --- CORRECCIÓN DE TYPEWRITER A SCRUB ---
        gsap.utils.toArray('.typewriter-group').forEach(group => {
            const text = group.textContent.trim();
            group.textContent = ''; // Inicia el texto vacío

            // La animación ahora está directamente vinculada al ScrollTrigger con scrub
            gsap.to(group, {
                text: { value: text },
                ease: "none",
                scrollTrigger: {
                    ...
                    scrub: 1.5 // Ajusta este valor para controlar la "sensibilidad" del scrub
                }
            });
        });
