...
### 2. Cambios y Mejoras en Strapi (La Versión Final)

Mantenemos la idea del componente repetible porque es potente, pero vamos a mejorarlo para que se alinee con nuestra nueva "librería de animaciones".

**Paso 1: Modifica tu Componente `TextGroup`**

Ve a **Content-Type Builder** y edita tu componente `TextGroup`. Vamos a hacer los siguientes ajustes:

1.  **Elimina campos viejos:** Borra los campos `style`, `background_color`, `text_color`, `icon_position` y `icon_animation`. Ya no los necesitamos porque los estilos y posiciones ahora están fijos en el código para máxima fidelidad.
2.  **Añade un campo de texto `text_1`:** Si no lo tienes, añade un campo de tipo **Text** (short text) llamado `text_1`.
3.  **Añade más campos de texto (Opcional):** Para grupos complejos como "Nice and Easy", añade campos de texto `text_2` y `text_3`.
4.  **Añade un campo de Media `icon`:** Añade un campo de tipo **Media** (single media) llamado `icon`.
5.  **El Campo Clave - `animation_type`:**
    * Elimina el campo `animation_in` si lo tenías.
    * Crea un nuevo campo de tipo **Enumeration** y llámalo `animation_type`.
    * En sus valores, pon los nombres de las funciones de nuestra `animationLibrary`. Por ahora:
      ```
      default-fade-in
      nice-group-entry
      icon-hand-float
