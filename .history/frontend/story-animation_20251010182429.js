...
export function buildDynamicStorySection(scene, mainContainer) {
    ...
    const styles = `
        ...
        .home-animate__text-group{ align-items:center; display:flex; font-size:max(5.625rem,min(5.58824vw + 1.29412rem,8rem)); font-weight:600; line-height:1; position:relative; margin: 0 2.5vw; white-space:nowrap; }
        ...
        /* --- CORRECCIÓN DE ICONO --- */
        .home-animate__icon { width: 6rem; height: auto; display: inline-block; vertical-align: middle; margin: 0 1rem; opacity: 0; }
        .default-group, .typewriter-group { font-family: 'Playfair Display', serif; }
    `;
    ...
    textGroupsData.forEach(groupData => {
        ...
        // --- CORRECCIÓN DE ICONO ---
             ...
             groupHTML = `<p class="home-animate__text-group icon-group" style="opacity:0;">
        ...
        // --- NUEVA ANIMACIÓN: TYPEWRITER ---
            ...
            groupHTML = `<p class="home-animate__text-group typewriter-group" style="opacity:0;">${groupData.text_1 || ''}</p>`;
        }
        else { // Animación por defecto
            groupHTML = `<p class="home-animate__text-group default-group" style="opacity: 0;">${groupData.text_1 || ''}</p>`;
        }
    ...
    requestAnimationFrame(() => {
        ...
        gsap.utils.toArray('.icon-group .home-animate__icon').forEach(icon => {
            gsap.from(icon, { rotation:-45, yPercent:100, opacity:0, scrollTrigger: { trigger: icon.parentElement, containerAnimation:scrollTl, start:"left 80%", end:"left center", scrub:1 } });
        });
        ...
        gsap.utils.toArray('.typewriter-group').forEach(group => {
            ...
            ScrollTrigger.create({
                ...
                onEnter: () => gsap.to(group, { text: { value: text, speed: 1 }, ease: "none" }),
                onLeaveBack: () => group.textContent = '',
            });
        });
