...
export function buildDynamicStorySection(scene, mainContainer) {
    ...
    // --- DEPURACIÓN DE DATOS ---
    console.log("--- DEBUGGING STORY SECTION DATA ---");
    textGroupsData.forEach((groupData, index) => {
        console.log(`[${index}] Group Data Received:`, JSON.parse(JSON.stringify(groupData)));
        if (groupData.animation_type === 'icon-hand-float') {
            console.log(`[${index}] Icon object inspection:`, groupData.icon);
        }
    });
    console.log("------------------------------------");
        ...
        if (groupData.animation_type === 'nice-group-entry') {
            groupHTML = `<p class="home-animate__text-group home-animate__text-group--nice" aria-hidden="true">
             ...
             groupHTML = `<p class="home-animate__text-group icon-group">
                ...
                <img class="home-animate__icon" src="${iconUrl}" alt="${iconData.alternativeText || 'icon'}" onload="console.log('✅ Icon loaded: ${iconUrl}')" onerror="console.error('❌ Icon FAILED to load: ${iconUrl}')"/>
    ...
    requestAnimationFrame(() => {
        ...
        iconGroups.forEach((group, index) => {
            ...
            if (!icon) {
                console.warn(`⚠️ No icon element found in group ${index}`);
                return;
            }
            ...
            gsap.from(icon, { 
                ...
                scrollTrigger: { 
                    ...
                    scrub:1,
                    onEnter: () => console.log(`▶️ Icon ${index} animation entered`),
                    onUpdate: (self) => console.log(`🔄 Icon ${index} progress: ${self.progress.toFixed(2)}`),
                } 
            });
        });
